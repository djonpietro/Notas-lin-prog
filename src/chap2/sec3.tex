...
\begin{def:independencia convexa}
	Seja $P \subset \mathbb{R}^n$ é um conjunto de pontos. $P$ é convexo
	independente se nenhum de seus pontos pode ser dado como combinação
	convexa de outros dois.
...
\begin{thm:caratheodory}[Carathéodory]
	...
	\begin{proof}
		...
		donde concluímos que $x$ pode ser escrito como combinação convexa de $k - 1$ pontos de $P$
		
		Como podemos repetir esse processo enquanto $k > n + 1$, ou seja, enquanto o lema \ref{lemma:afim}
		pode ser aplicado, então $x$ pode ser escrito como combinação convexa de até $n + 1$ pontos de $P$.
	\end{proof}
